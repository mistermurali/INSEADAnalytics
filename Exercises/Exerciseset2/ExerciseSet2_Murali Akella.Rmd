...
sorted_cols <- sort(Rotated_Factors[,2], decreasing = TRUE, index.return = TRUE)$ix
Rotated_Factors <- Rotated_Factors[sorted_rows,]
#Rotated_Factors <- Rotated_Factors[sorted_cols,]
Rotated_Factors[abs(Rotated_Factors) < 0.3]<-NA
#Highest_weights <- c(head(Rotated_Factors,5))
```
...
*
A1) 6, at which 52.29% is explained

A2) They are 0.93, 0.93, 0.86, 0.85, 0.79
## 

A3) The first 10 components are the top 10 selected factors that explain the variance in the data, they can be interpreted as:

Government bonds
G10
European stock markets
European indices (diversified)
American indices (diversified)
European Crosses (Rates)
Oil and derived commodities
Base metals
...
print(m1,'chart')
```
In the bull market there are 8 principal components and the in the bear market only 4 (however with more variables). Aassets become much more correlated during the crisis.
...
**Answers**
1. This is an investment strategy that shows an investment strategy in line with the x-day moving averages of the futures series 
