...
**Your Answers here:**
<br>
This is done while downloading the source in dataSet1.R - downloading ticker AAPL..
<br>
We have `r nrow(StockReturns)` days of data, starting from `r rownames(StockReturns)[2]` until `r tail(rownames(StockReturns),2)`.  Here are some basic statistics about Apple stock:
...
2. The standard deviation of the daily returns of Apple during this period is `r round(100*sd(StockReturns[,2]),3)`%;

Here are returns of Apple in this period:
...
```
<br>
Yes, simply change the myTickers vector in dataSet1.R to include Yahoo! (YHOO) and this will enable all data / reports as for S&P to be downloaded as well, taking care to also update length of mytickers <br>
